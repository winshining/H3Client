...
static HuffmanEncodeCode  HuffmanEncodeTable[256] =
{
...
static HuffmanEncodeCode  HuffmanEncodeTableLC[256] =
{
    {0x00001ff8, 13}, {0x007fffd8, 23}, {0x0fffffe2, 28}, {0x0fffffe3, 28},
    {0x0fffffe4, 28}, {0x0fffffe5, 28}, {0x0fffffe6, 28}, {0x0fffffe7, 28},
    {0x0fffffe8, 28}, {0x00ffffea, 24}, {0x3ffffffc, 30}, {0x0fffffe9, 28},
...
#define HuffmanEncodeBuf(dst, buf)                                             \
    ((dst)[0] = (uint8_t) ((buf) >> 56),                                       \
     (dst)[1] = (uint8_t) ((buf) >> 48),                                       \
     (dst)[2] = (uint8_t) ((buf) >> 40),                                       \
     (dst)[3] = (uint8_t) ((buf) >> 32),                                       \
     (dst)[4] = (uint8_t) ((buf) >> 24),                                       \
     (dst)[5] = (uint8_t) ((buf) >> 16),                                       \
     (dst)[6] = (uint8_t) ((buf) >> 8),                                        \
     (dst)[7] = (uint8_t)  (buf))
...
size_t
HuffmanEncode(uint8_t *src, size_t len, uint8_t *dst, bool lower)
{
    uint8_t            *end;
    size_t              hlen;
#if (PTR_SIZE == 8)
...
#else
    uint32_t            buf, pending, code;
#endif
    HuffmanEncodeCode  *table, *next;

    table = lower ? HuffmanEncodeTableLC
                  : HuffmanEncodeTable;
    hlen = 0;
    buf = 0;
    pending = 0;
