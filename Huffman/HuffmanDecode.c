...
static inline bool HuffmanDecodeBits(uint8_t *state, uint8_t *ending, uint32_t bits, uint8_t **dst);

static HuffmanDecodeCode  HuffmanDecodeCodes[256][16] =
{
    /* 0 */
    {
...
bool HuffmanDecode(uint8_t *state, uint8_t *src, size_t len, uint8_t **dst, bool last)
{
    ...
    while (src != end) {
        ...
        if (!HuffmanDecodeBits(state, &ending, ch >> 4, dst)) {
            printf("huffman decoding error at state %d: bad code 0x%Xd",
                   *state, ch >> 4);

            return false;
        }

        if (!HuffmanDecodeBits(state, &ending, ch & 0xf, dst)) {
            printf("huffman decoding error at state %d: bad code 0x%Xd",
                   *state, ch & 0xf);
    ...
    if (last) {
        if (!ending) {
            printf("huffman decoding error: incomplete code 0x%Xd", ch);
...
static inline bool HuffmanDecodeBits(uint8_t *state, uint8_t *ending, uint32_t bits, uint8_t **dst)
{
    HuffmanDecodeCode  code;

    code = HuffmanDecodeCodes[*state][bits];
